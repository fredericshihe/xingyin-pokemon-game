...
| 第 1 期：数据模型与迁移 | 本地已完成，待部署 | 2026-07-20 | schema v5、客户端单调合并、旧玩家豁免及 `202607200002_preserve_long_term_progression.sql` 已完成；迁移未推送远端，因此现有线上记录未被改写 |
| 第 2 期：完成度页面与永久图鉴 | 本地已完成 | 2026-07-20 | 十四章地图册、HUD/P 键入口、原子目标分类及永久图鉴三类状态完成；桌面/390px 窄屏无横向溢出，页面交互验收正常 |
| 第 3 期：阶段奖励 | 本地已完成，待远端灰度 | 2026-07-21 | 四阶段奖励、成功后演出、防重入及唯一领取事实完成；专用原子奖励 RPC 已接入并通过本机 PostgreSQL 双连接真实竞争测试，远端迁移、生产备份恢复和两台真实终端回归仍是部署前闸门 |
