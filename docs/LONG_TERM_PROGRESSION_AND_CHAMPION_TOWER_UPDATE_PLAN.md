...
| 第 1 期：数据模型与迁移 | staging 已完成，生产待部署 | 2026-07-21 | staging 已按顺序执行至 `202607210001`；未复制生产玩家数据，生产迁移历史仍停在 `202607200001` |
...
| 第 6 期：灰度与正式发布 | 门禁阻断，未部署生产 | 2026-07-21 | 备份恢复和 staging 新函数回归通过；全库 `plpgsql_check` 被既有失效函数阻断，且当前环境只有一种浏览器引擎，未满足“双浏览器”硬门禁 |
