...
await withViteAuditServer(async ({ rootDir, loadModule }) => {
  ...
  const checks = [
    ...
    { name: 'camera_uses_rendered_impact_without_forced_freeze', passed: /event.playback\?\.impact\(\)/.test(originalGame) && /battle-cinematic--camera-/.test(originalGame) && /const hitStopMs = 0/.test(cinematics) },
    ...
    { name: 'effect_renderer_uses_preloaded_material_atlas_without_glyphs', passed: /preloadBattleVfxAtlas/.test(moveEffect) && /battle-vfx-atlas\.png/.test(moveEffect) && !/battle-vfx-icon|battle-vfx-symbol/.test(moveEffect) },
    ...
    { name: 'reduced_motion_covers_battle_cinematics', passed: /prefers-reduced-motion:\s*reduce[\s\S]*?battle-cinematic/.test(css) && /prefers-reduced-motion: reduce/.test(moveEffect) && /createBattleActorPlayback[^;]+reducedMotion/.test(moveEffect) },
