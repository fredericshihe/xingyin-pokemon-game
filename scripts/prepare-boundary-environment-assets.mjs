...
const folder='public/assets/3d/map-boundaries-v5'
...
for(const item of source){
 ...
 const id=`boundary_${item.name}`;const bytes=(await fs.stat(file)).size
 ...
 catalog.push({id,name:item.label,ownerMap:item.mapId,sourcePackage:'xingyin-map-exclusive-blender',assetPath:`/assets/3d/map-boundaries-v5/${item.name}.glb`,sourceAssetName:`${item.name}.glb`,themeTags:[item.setId,'exclusive-scenery'],footprint,defaultScale:1,defaultBlocking:false,heightClass:bounds.max[1]>2?'high':'medium'})
...
if(new Set(manifest.map(m=>m.geometryHash)).size!==manifest.length)throw new Error('Duplicate geometry: '+JSON.stringify(manifest.filter((m,i)=>manifest.findIndex(n=>n.geometryHash===m.geometryHash)!==i).map(m=>m.name)))
await fs.writeFile(`${folder}/manifest.json`,JSON.stringify(manifest,null,2)+'\n')
await fs.writeFile('src/game/data/mapBoundaryAssets.generated.js',`// Blender-authored map-specific scenery. Rebuild: scripts/blender/build_biome_boundaries.py\nexport const MAP_BOUNDARY_ASSETS = ${JSON.stringify(catalog,null,2)}\nexport const MAP_BOUNDARY_SETS = ${JSON.stringify(sets,null,2)}\nexport const MAP_BOUNDARY_MEASUREMENTS = ${JSON.stringify(measurements,null,2)}\n`)
