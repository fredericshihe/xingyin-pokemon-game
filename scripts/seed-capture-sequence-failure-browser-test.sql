-- Seeds a temporary browser-test student directly into a failed capture animation.
-- Expected front-end state after login:
-- - Capture animation plays for 独角虫.
-- - After completion, the game remains in battle and hands the turn to the enemy.
...
BEGIN
  ...
  VALUES (
    ...
    jsonb_build_object(
      ...
      'view', 'battle',
      'turn', 'capture',
      'logs', jsonb_build_array('测试：精灵球正在捕捉独角虫，但它会挣脱。'),
