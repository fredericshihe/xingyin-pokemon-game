-- Save cloud snapshot and resource deltas in one transaction so purchases and
-- battle starts do not split backend resources from game progress.
...
BEGIN
  ...
  IF p_energy_delta <> 0 THEN
    v_current_energy := v_current_energy + p_energy_delta;
    UPDATE users
    ...
    VALUES (p_user_id, p_energy_delta, p_energy_reason, v_current_energy, v_max_energy);
  END IF;
