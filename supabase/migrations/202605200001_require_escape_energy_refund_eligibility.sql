-- Tighten student-side escape energy refunds.
--
-- The front end now tracks whether a wild battle is still eligible for an
-- energy refund. A refund is valid only when the player escaped before taking
-- any battle action. The database must enforce the same contract instead of
-- trusting only the requested resource delta.
