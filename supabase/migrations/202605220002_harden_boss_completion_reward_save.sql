-- Harden boss completion persistence against reward-phase saves and later stale
-- client snapshots. Boss victories are permanent progress, so later saves must
-- not be able to remove already recorded boss ids.
...
BEGIN
  ...
  v_is_completed_boss_save := (
    COALESCE(p_force_complete, FALSE)
  ...
  IF NOT (v_defeated_boss_ids ? v_scoped_id) THEN
...
IMMUTABLE
AS $$
  SELECT apply_configured_boss_completion_to_game_data($1, FALSE);
$$;
...
DECLARE
  ...
  v_game_data JSONB := apply_configured_boss_completion_to_game_data(p_game_data, FALSE);
