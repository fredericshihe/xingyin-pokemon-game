-- Treat heartbeats as a bounded server lease: stale leases stop accruing and cannot write.

BEGIN;

ALTER TABLE student_playtime_sessions
...
BEGIN
  ...
  ELSIF v_session_active AND v_last_heartbeat_at IS NOT NULL THEN
    ...
    v_lease_stale := v_elapsed_seconds > v_max_heartbeat_gap_seconds;

    IF v_lease_stale THEN
    ...
    IF v_lease_stale THEN
      ...
      v_session_active := FALSE;
      v_last_heartbeat_at := v_now;
    ELSIF v_elapsed_seconds >= 1 THEN
      UPDATE student_playtime_sessions s
  ...
  ELSIF v_action = 'end' AND v_session_id = v_requested_session_id THEN
    ...
    SET active = FALSE,
        ...
        updated_at = v_now,
        ended_at = CASE
          WHEN v_session_active THEN v_now
          ELSE s.ended_at
        END
    WHERE s.student_id = p_student_id;
    v_session_active := FALSE;
...
AS $$
  SELECT EXISTS (
    ...
    WHERE s.student_id = p_student_id
      ...
      AND s.session_id = LEFT(COALESCE(NULLIF(TRIM(COALESCE(p_session_id, '')), ''), 'legacy'), 128)
      AND (
        s.active
        OR (
          p_allow_recent_end
...
REVOKE ALL ON FUNCTION student_playtime_lease_is_valid(UUID, TEXT, BOOLEAN) FROM PUBLIC, anon, authenticated;
...
BEGIN
  ...
  IF COALESCE(v_status.remaining_seconds, 0) <= 0 THEN
    RETURN json_build_object('success', false, 'error', '今日游玩时间已用完。');
  END IF;
  ...
  IF NOT student_playtime_lease_is_valid(p_user_id, v_session_id, TRUE) THEN
    RETURN json_build_object('success', false, 'error', '无法确认当前游玩会话，请重新校验时长。');
  END IF;
...
BEGIN
  ...
  IF NOT student_playtime_lease_is_valid(p_student_id, v_session_id, TRUE) THEN
    RETURN;
  END IF;
...
DECLARE
  ...
  v_session_id TEXT := LEFT(
    COALESCE(NULLIF(p_game_data #>> '{_sync,playtimeSessionId}', ''), 'legacy:' || p_user_id::TEXT),
    128
  );
  v_game_data JSONB;
...
BEGIN
  ...
  IF NOT student_playtime_lease_is_valid(p_user_id, v_session_id, TRUE) THEN
    ...
    RETURN QUERY
    SELECT v_game_data, v_last_saved, v_save_revision, FALSE, '无法确认当前游玩会话，请重新校验时长。'::TEXT;
    RETURN;
  END IF;
...
BEGIN
  ...
  IF COALESCE(v_status.remaining_seconds, 0) <= 0 THEN
    RETURN QUERY
    SELECT v_game_data, v_last_saved, v_save_revision, FALSE, '今日游玩时间已用完。'::TEXT, v_gold, v_energy, v_max_energy;
    RETURN;
  END IF;

  IF NOT student_playtime_lease_is_valid(p_user_id, v_session_id, TRUE) THEN
    RETURN QUERY
    SELECT v_game_data, v_last_saved, v_save_revision, FALSE, '无法确认当前游玩会话，请重新校验时长。'::TEXT, v_gold, v_energy, v_max_energy;
    RETURN;
  END IF;
